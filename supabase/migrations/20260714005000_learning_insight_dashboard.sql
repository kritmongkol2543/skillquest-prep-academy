create index if not exists "Log_UserID_CategoryID_idx"
  on public."Log" ("UserID", "CategoryID", "TimeStamp" desc);

create index if not exists "Log_UserID_TestID_idx"
  on public."Log" ("UserID", "TestID", "TimeStamp" desc);
...
as $$
  ...
  overview as (
    select
      coalesce(count(distinct l."QuestionID"), 0)::integer as questions_seen,
      coalesce(sum(l."Duration"), 0)::bigint as total_question_seconds,
      coalesce(round(avg(nullif(l."Duration", 0))), 0)::integer as avg_seconds_per_question,
      coalesce(percentile_disc(0.5) within group (order by nullif(l."Duration", 0)), 0)::integer as median_seconds_per_question,
      coalesce(count(distinct a."AttemptID"), 0)::integer as attempts_count,
      coalesce(round(avg(a."Accuracy"), 2), 0)::numeric as average_accuracy,
      coalesce(sum(a."HintCount"), 0)::integer as hint_count,
      max(l."TimeStamp") as last_activity_at
    from logs l
    full join attempts a on false
  ),
  subject_rows as (
    select
