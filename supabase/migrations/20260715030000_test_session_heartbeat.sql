alter table public."Test"
  add column if not exists "ClientInstanceID" uuid,
  add column if not exists "HeartbeatAt" timestamptz;

update public."Test"
set "HeartbeatAt" = coalesce("HeartbeatAt", "UpdatedAt", "StartedAt", now())
where "Status" in ('in_progress', 'paused')
  and "HeartbeatAt" is null;

create index if not exists "Test_Active_HeartbeatAt_idx"
  on public."Test" ("HeartbeatAt")
  where "Status" in ('in_progress', 'paused');

create or replace function public.expire_stale_test_sessions()
returns integer
language plpgsql
security definer
set search_path = ''
...
begin
  ...
  set
    ...
    "UpdatedAt" = now()
  where "Status" in ('in_progress', 'paused')
    and coalesce("HeartbeatAt", "UpdatedAt", "StartedAt") < now() - interval '15 minutes';
...
begin
  ...
  -- One shared practice desk: expire absent pages, then serialise every start.
  perform pg_catalog.pg_advisory_xact_lock(739241);
  perform public.expire_stale_test_sessions();

  select
    t."TestID", t."Subject", t."Category", t."Status", t."StartedAt", t."QuestionCount",
  ...
  insert into public."Test" (
    ...
    v_first_question."QuestionID", v_category."Subject", 'ชุดที่ ' || v_category."Category",
...
create or replace function public.heartbeat_test_service(
  p_user_id uuid,
  p_test_id uuid,
  p_client_instance_id uuid
)
...
begin
  ...
  where "TestID" = p_test_id
    ...
    and "ClientInstanceID" = p_client_instance_id
    and "Status" in ('in_progress', 'paused')
  returning * into v_test;

  if not found then raise exception 'SET_NOT_ACTIVE'; end if;
  return jsonb_build_object(
    'test_id', v_test."TestID",
    'status', v_test."Status",
    'heartbeat_at', v_test."HeartbeatAt"
  );
end;
$$;
...
begin
  update public."Test"
  set "Status" = 'paused', "PausedAt" = now(), "HeartbeatAt" = now(), "UpdatedAt" = now()
  where "TestID" = p_test_id
    and "UserID" = p_user_id
    and "Status" in ('in_progress', 'paused')
  returning * into v_test;
  ...
  if not found then raise exception 'SET_NOT_AVAILABLE'; end if;
  return jsonb_build_object('test_id', v_test."TestID", 'status', v_test."Status", 'paused_at', v_test."PausedAt");
end;
$$;

revoke all on function public.expire_stale_test_sessions() from public, anon, authenticated;
grant execute on function public.expire_stale_test_sessions() to service_role;
...
revoke all on function public.heartbeat_test_service(uuid, uuid, uuid) from public, anon, authenticated;
grant execute on function public.heartbeat_test_service(uuid, uuid, uuid) to service_role;
